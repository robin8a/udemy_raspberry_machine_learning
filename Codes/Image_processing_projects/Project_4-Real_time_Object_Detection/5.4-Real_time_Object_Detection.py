...
def ORB(input_image, stored_image):
    ...
    orb_detector = cv2.ORB_create(1600, 1.3)        # Initiate SIFT detector
...
while True:
	...
	x1_top_left = int(frame_width / 3)
	y1_top_left = int((frame_height / 2) + (frame_height / 4))
	x2_bottom_right = int((frame_width / 3) * 2)
	y2_bottom_right = int((frame_height / 2) - (frame_height / 4))
